...
public class CreateBugPage extends AbstractLoggedinBugzillaPage {
	...
	/**
	 * edited for new products exist - title changes
	 */
	@Override
	protected boolean isMatchingPage() {
		return ("Enter Bug".equals(getTitle()) || getTitle().matches("Enter Bug: .*")); 
	}
	
	public CreateBugPage selectProductForBug(String productName){
		//Only sure at this point that product exists
		driver.findElement(By.cssSelector("a[href*='"+ productName +"']")).click();
